...
//
// Mirrors bridge/dashboard/web/src/components/FlowFields.tsx (the ONE difference
...
function Shot({ path, onZoom }: { path: string; onZoom: (src: string) => void }) {
  ...
  if (!src) return null;
  return (
    <button type="button" onClick={() => onZoom(src)} aria-label={`Open ${path}`} className="block">
      <img
        src={src}
        alt={path}
        className="h-28 w-auto max-w-[200px] rounded-lg border border-border object-cover"
      />
    </button>
  );
}
