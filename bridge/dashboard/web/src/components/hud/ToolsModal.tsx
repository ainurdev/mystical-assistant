...
const LABEL = { fontSize: 9, letterSpacing: 1.5, color: "var(--txl)", margin: "0 0 9px" };
  ...
  const row = (key: string, name: string, hint: string, rule: string, dim?: boolean) => {
    ...
    return (
      <div key={key}
        ...
        <span style={{ fontSize: 12, color: "var(--txb)", minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{name}</span>
        <span style={{ fontSize: 9, color: dim ? "var(--warn)" : "var(--txd)", minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{hint}</span>
        <span style={{ flex: 1 }} />
    ...
    <div onClick={onClose}
      ...
      <div onClick={(e) => e.stopPropagation()} className="panel"
        ...
        <div className="mscroll" style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "14px 18px" }}>
          <div style={{ fontSize: 9, letterSpacing: 1.5, color: "var(--txl)", marginBottom: 14 }}>
          ...
          <div style={box}>
            {sets === null && (
              <div style={{ padding: "11px 12px", fontSize: 11, color: "var(--txd)" }}>reading `claude mcp list`…</div>
            )}
            {sets?.servers.length === 0 && (
              <div style={{ padding: "11px 12px", fontSize: 11, color: "var(--txd)" }}>no MCP servers configured</div>
            )}
            {(sets?.servers ?? []).map((s) =>
