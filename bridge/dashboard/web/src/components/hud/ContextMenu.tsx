...
export function ContextMenu({ ctx, items, closing, onClose }: {
  ...
  return (
    <>
      ...
      <div data-ctxmenu="1"
        style={{ position: "fixed", left: x, top: y, zIndex: 97, width: W, border: "1px solid rgba(127,233,216,.4)", background: "rgba(7,13,13,.98)", boxShadow: "0 12px 44px rgba(0,0,0,.7),0 0 26px rgba(127,233,216,.08)", transformOrigin: "top", animation: closing ? "ctxOut .165s ease both" : "ctxIn .28s cubic-bezier(.16,.84,.3,1) both" }}>
          ...
          {items.map((it, i) =>
            it.divider ? (
              ...
              <button key={i} onClick={() => { it.onClick?.(); onClose(); }}
                ...
                style={{ width: "100%", appearance: "none", cursor: "pointer", border: 0, background: hov === i ? (it.danger ? "rgba(224,137,122,.1)" : "rgba(127,233,216,.08)") : "transparent", color: it.danger ? "#cf9387" : "#cfe9e3", fontFamily: "inherit", fontSize: 11, letterSpacing: ".3px", textAlign: "left", display: "flex", alignItems: "center", gap: 10, padding: "7px 12px" }}>
