...
export function AnalyzeModal(props: Props) {
  ...
  const tabs: { k: Tab; l: string; badge?: number }[] = [
    ...
    { k: "changes", l: "CHANGES", badge: git?.dirty || undefined },
  ...
  return (
    <div onClick={close}
      ...
      <div onClick={(e) => e.stopPropagation()} className="panel"
        style={{ width: full ? "96vw" : 880, maxWidth: full ? "96vw" : "94vw", height: full ? "94vh" : undefined, maxHeight: full ? "94vh" : "84vh", display: "flex", flexDirection: "column", border: "1px solid color-mix(in srgb, var(--acc) 40%, transparent)", background: "color-mix(in srgb, var(--panel2) 98%, transparent)", boxShadow: "0 0 70px rgba(0,0,0,.75),0 0 30px color-mix(in srgb, var(--acc) 8%, transparent)", transition: "width .24s cubic-bezier(.16,.84,.3,1), max-width .24s cubic-bezier(.16,.84,.3,1), max-height .24s cubic-bezier(.16,.84,.3,1)", animation: closing ? "modalOut .28s ease-in forwards" : "modalIn .46s cubic-bezier(.16,.84,.3,1) both" }}>
        {/* header */}
        <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "14px 18px", borderBottom: "1px solid color-mix(in srgb, var(--acc) 16%, transparent)", flex: "none", flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {TAG_COLORS.map((c) => (
                    <button key={c.color} onClick={() => setTagColorDraft(c.color)}
                      style={{ width: 20, height: 20, borderRadius: "50%", border: 0, cursor: "pointer", background: c.color, boxShadow: tagColorDraft === c.color ? `0 0 0 2px var(--panel3), 0 0 0 3px ${c.color}` : "0 0 0 0 transparent", padding: 0, flex: "none" }} />
                  ))}
                </div>
          ...
          <span style={{ flex: 1 }} />
          <button onClick={() => setFull((f) => !f)} title={full ? "restore size" : "expand to full screen"} {...hp("full")}
            style={{ appearance: "none", cursor: "pointer", border: "1px solid color-mix(in srgb, var(--acc) 25%, transparent)", background: hov === "full" ? "color-mix(in srgb, var(--acc) 8%, transparent)" : "transparent", color: hov === "full" ? "var(--txb)" : "var(--txm)", fontFamily: "inherit", padding: "4px 9px", display: "flex", alignItems: "center", lineHeight: 0 }}>
            {full ? (
        ...
        {/* body — flex column so fill-tabs (changes/editor/terminal/issues) can flex:1
            to a definite height and scroll their own panes; scroll-tabs stay auto-height */}
        <div className="mscroll" style={{ flex: 1, overflowY: "auto", padding: 18, display: "flex", flexDirection: "column" }}>
          {tab === "overview" && (
            <OverviewTab project={project} git={git} dotColor={tagColor} issueCount={issueCount}
              ...
              attachedSessionId={props.attachedSessionId} />
          )}
          {tab === "changes" && (
...
const SURF_FALLBACK = { c: "var(--txm)", bd: "rgba(159,199,192,.4)" };
...
function OverviewTab({ project, git, dotColor, issueCount, worktrees, branches, sessions, status, onGoto, onFeed, onSelectSession, onNewSession, onWorktreeSession, attachedSessionId }: {
            ...
            {branchList.map((b) => {
              ...
              return (
                <button key={b} onClick={() => { setNsBranch(b); setNsNewOpen(false); }} {...hp(`nsb:${b}`)}
                  style={{ appearance: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, border: `1px solid ${on || hov === `nsb:${b}` ? "var(--purple)" : "color-mix(in srgb, var(--acc) 18%, transparent)"}`, background: on ? "color-mix(in srgb, var(--purple) 16%, transparent)" : "transparent", color: on ? "var(--purple-b)" : "var(--txm)", fontFamily: "'JetBrains Mono',monospace", fontSize: 9.5, padding: "5px 8px" }}>
                ...
                {open && (
                  <div style={{ borderTop: "1px solid color-mix(in srgb, var(--acc) 10%, transparent)", padding: "11px 11px 12px", animation: "mslide .2s ease both" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      {s.id === attachedSessionId && <span style={{ fontSize: 8.5, letterSpacing: 1, color: "var(--acc)" }}>● ATTACHED</span>}
                      <span style={{ flex: 1 }} />
...
function ChangesTab({ project, branch, branchOpts, onPickBranch, onRefreshGit }: {
  ...
  return (
    <div style={{ animation: "mslide .3s ease both", flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ fontSize: 9.5, letterSpacing: 1.5, color: "var(--txl)", marginBottom: 11, flex: "none" }}>WORKING TREE</div>
      {st && files.length === 0 && (
        <div style={{ fontSize: 12, color: "var(--txd)", fontFamily: "'JetBrains Mono',monospace", padding: "6px 2px" }}>Working tree clean.</div>
      )}
      {files.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", border: "1px solid color-mix(in srgb, var(--acc) 12%, transparent)", minHeight: 362, flex: 1 }}>
          {/* file list */}
          <div style={{ borderRight: "1px solid color-mix(in srgb, var(--acc) 12%, transparent)", display: "flex", flexDirection: "column", minHeight: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 12px 8px", flex: "none" }}>
              <span style={{ fontSize: 9, letterSpacing: 1.5, color: "var(--txl)" }}>CHANGED · {files.length}</span>
              <span style={{ flex: 1 }} />
              ...
              {files.map((f) => {
                ...
                return (
                  <button key={f.path} onClick={() => setSel(f.path)} {...hp(`f:${f.path}`)}
                    style={{ width: "100%", appearance: "none", border: 0, borderLeft: `2px solid ${on ? "var(--acc)" : "transparent"}`, background: on ? "color-mix(in srgb, var(--acc) 8%, transparent)" : hov === `f:${f.path}` ? "color-mix(in srgb, var(--acc) 5%, transparent)" : "transparent", cursor: "pointer", fontFamily: "'JetBrains Mono',monospace", textAlign: "left", display: "flex", alignItems: "center", gap: 9, padding: "8px 10px" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, width: 13, textAlign: "center", flex: "none", color: FILE_COLOR(f.status) }}>{f.status}</span>
                    <span style={{ fontSize: 11, color: on ? "var(--txb)" : "var(--txm)", flex: 1, minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", direction: "rtl", textAlign: "left" }}>{f.path}</span>
            ...
            <div style={{ borderTop: "1px solid color-mix(in srgb, var(--acc) 10%, transparent)", flex: "none" }}>
              ...
              <div style={{ padding: "8px 9px 6px" }}>
                <button onClick={() => void doCommit()} {...hp("commit")}
                  style={{ width: "100%", appearance: "none", cursor: "pointer", border: "1px solid var(--acc)", background: hov === "commit" ? "color-mix(in srgb, var(--acc) 22%, transparent)" : "color-mix(in srgb, var(--acc) 12%, transparent)", color: "var(--txb)", fontFamily: "inherit", fontSize: 10, letterSpacing: 1.5, padding: 8 }}>COMMIT ALL</button>
              </div>
...
function IssuesTab({ project, info, onFeed, onReload }: {
  ...
  return (
    <div style={{ animation: "mslide .3s ease both", flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      ...
      {/* master-detail */}
      <div style={{ display: "grid", gridTemplateColumns: "330px 1fr", border: "1px solid color-mix(in srgb, var(--acc) 12%, transparent)", minHeight: 372, flex: 1 }}>
        {/* LEFT: list */}
        <div style={{ borderRight: "1px solid color-mix(in srgb, var(--acc) 12%, transparent)", display: "flex", flexDirection: "column", minHeight: 0 }}>
...
function TerminalTab({ project, worktrees, branch, onCount }: {
  ...
  return (
    <div style={{ animation: "mslide .3s ease both", flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      {/* run project */}
      <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 11, border: "1px solid color-mix(in srgb, var(--ok) 24%, transparent)", background: "color-mix(in srgb, var(--ok) 4%, transparent)", padding: "9px 11px", flexWrap: "wrap", flex: "none" }}>
