    ...
    >
      ...
      >
        ...
        <span style={{ flex: 1, minWidth: 0, display: "block" }}>
          <span style={{ display: "block", fontSize: "var(--t125)", color: "var(--txb)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{g.name}</span>
          <span style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3, fontSize: "var(--t9)", color: "var(--txd)" }}>
      ...
      {g.running && (
        <span style={{ fontSize: "var(--t8)", letterSpacing: 1, color: "var(--ok)", border: "1px solid color-mix(in srgb, var(--ok) 30%, transparent)", padding: "1px 5px", flex: "none" }}>LIVE</span>
      )}
      <button
  ...
  return (
    <div className="panel" style={{ border: "1px solid color-mix(in srgb, var(--acc) 16%, transparent)", background: "color-mix(in srgb, var(--panel) 86%, transparent)", flex: "none" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px" }}>
        <span style={{ fontSize: "var(--t105)", letterSpacing: 2.5, color: "var(--txl)" }}>PROJECTS</span>
        <span style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <span style={{ fontSize: "var(--t95)", letterSpacing: 1.5, color: "var(--acc)" }}>{needle ? `${shown.length}/${groups.length}` : shown.length} REPOS</span>
          <button
            onClick={onManage}
            title="manage projects — hide, remove, import"
            onMouseEnter={() => setManageHov(true)} onMouseLeave={() => setManageHov(false)}
            style={{ appearance: "none", cursor: "pointer", border: "1px solid color-mix(in srgb, var(--acc) 25%, transparent)", background: manageHov ? "color-mix(in srgb, var(--acc) 14%, transparent)" : "color-mix(in srgb, var(--acc) 5%, transparent)", color: manageHov ? "var(--txb)" : "var(--txm)", fontFamily: "inherit", fontSize: "var(--t9)", letterSpacing: 1, padding: "3px 8px" }}
          >MANAGE</button>
          <button
            onClick={() => setNewOpen((o) => !o)} title="new project from a prompt"
            onMouseEnter={() => setNewHov(true)} onMouseLeave={() => setNewHov(false)}
            style={{ appearance: "none", cursor: "pointer", border: "1px solid color-mix(in srgb, var(--info) 32%, transparent)", background: newHov ? "color-mix(in srgb, var(--info) 16%, transparent)" : "color-mix(in srgb, var(--info) 6%, transparent)", color: "var(--info-hi)", fontFamily: "inherit", fontSize: "var(--t9)", letterSpacing: 1, padding: "3px 8px" }}
          >+ NEW</button>
        </span>
      </div>
      ...
      <div style={{ padding: "9px 10px 11px", display: "flex", flexDirection: "column", gap: 7 }}>
        <input
          ...
          style={{ width: "100%", boxSizing: "border-box", background: "color-mix(in srgb, var(--panel2) 45%, transparent)", border: "1px solid color-mix(in srgb, var(--acc) 16%, transparent)", outline: "none", color: "var(--txb)", fontFamily: "'JetBrains Mono',monospace", fontSize: "var(--t11)", padding: "6px 9px", marginBottom: 2 }} />
        {newOpen && (
          <div style={{ border: "1px solid color-mix(in srgb, var(--info) 32%, transparent)", background: "color-mix(in srgb, var(--info) 5%, transparent)", padding: "11px 12px", marginBottom: 10, animation: "mslide .2s ease both" }}>
            <div style={{ fontSize: "var(--t9)", letterSpacing: 1.5, color: "var(--txl)", marginBottom: 9 }}>NEW PROJECT</div>
            ...
            <div style={{ display: "flex", gap: 7, marginTop: 9 }}>
              <button
                ...
                style={{ flex: 1, appearance: "none", cursor: "pointer", border: "1px solid var(--info)", background: createHov ? "color-mix(in srgb, var(--info) 24%, transparent)" : "color-mix(in srgb, var(--info) 14%, transparent)", color: "var(--info-b)", fontFamily: "inherit", fontSize: "var(--t10)", letterSpacing: 1.5, padding: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                <span style={{ color: "var(--info)" }}>▸</span>CREATE & START</button>
              <button
                onClick={() => setNewOpen(false)}
                onMouseEnter={() => setCancelHov(true)} onMouseLeave={() => setCancelHov(false)}
                style={{ appearance: "none", cursor: "pointer", border: "1px solid color-mix(in srgb, var(--acc) 20%, transparent)", background: cancelHov ? "color-mix(in srgb, var(--acc) 6%, transparent)" : "transparent", color: "var(--txd)", fontFamily: "inherit", fontSize: "var(--t10)", letterSpacing: 1.5, padding: "8px 12px" }}>CANCEL</button>
            </div>
          </div>
        )}
        {clusterByParent(shown).map(({ parent, items }) => {
          ...
          return (
            <div key={parent} style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 4, marginLeft: (segs.length - 1) * 12 }}>
              <div style={{ fontSize: "var(--t85)", letterSpacing: 1.5, color: "var(--txd)", padding: "2px 2px 0" }}>
