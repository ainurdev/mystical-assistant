/* Pan/zoom arithmetic for the image lightbox, kept out of the component so it
 * can be checked without a DOM (imgzoom.check.ts).
 *
 ...
 *
 * Zoom is anchored, not centred: `zoomAt` holds the point under the cursor (or
 * the pinch midpoint) still, preserving `world = (p - x) / s` for that point —
 * the same identity lib/viewport.ts keeps for the canvas, at different stops.
