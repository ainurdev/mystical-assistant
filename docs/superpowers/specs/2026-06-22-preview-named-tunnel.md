# Preview via a stable named Cloudflare Tunnel — Design
...
Replace the `/preview` ephemeral `*.trycloudflare.com` quick tunnel with a **stable
named Cloudflare Tunnel** so the preview URL is always `https://preview.mhzrerfani.dev`.
...
| Provisioning | One-time, via the Cloudflare API (MCP): tunnel + DNS |

Rationale: dynamic port + no runtime API dependency ⇒ locally-managed. The bridge
owns a `config.yml` it rewrites per port and runs `cloudflared tunnel run`; no
Cloudflare API call happens at runtime (the MCP is a build-time tool only).

## 3. Provisioned resources (Cloudflare account `bd5c2f35…3924`)
...
- **Credentials file** `~/.cloudflared/mystical-preview.json` (mode 600, in `$HOME`,
...
- `_spawn_named(port)` runs `cloudflared tunnel --config <cfg> run`; a watch thread
...
it only runs on demand. When stopped, the hostname returns Cloudflare's tunnel-offline
